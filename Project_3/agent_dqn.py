...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        if args.test_dqn:
            ...
            self.net.load_state_dict(T.load("Gsrinivasan_DDQN.pth"))
...
##############################################################
    ...
    def save_network(self):
        ...
        T.save(self.net.state_dict(), "Gsrinivasan_DDQN.pth")    
        print("************************************************************")   
...
##############################################################
    def train(self):
        with open('Rewards.csv', mode='w') as dataFile:
                        rewardwriter = writer(dataFile)
                        rewardwriter.writerow(['Score'])
                        dataFile.close()
            ...
            if (done):
                ...
                if info['ale.lives'] == 0:
                    ...
                    with open('Rewards.csv', mode='a') as dataFile:
                        rewardwriter = writer(dataFile)
                        rewardwriter.writerow([mean_reward])
                        dataFile.close()
                    print("%d games, mean reward %.3f, epsilon %.2f" % (len(self.mean_reward), mean_reward, self.epsilon))
