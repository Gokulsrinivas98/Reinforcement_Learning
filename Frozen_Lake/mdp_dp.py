	...
	while True:
		...
		for state in range(0, nS):
			...
			for action in range(0, nA):
				for nextstate in P[state][action]:
					val += (policy[state][action]) * (nextstate[0] * (nextstate[2] + (gamma * value_function[nextstate[1]]))) #Bellman equation
			value_function_prev[state] = val	#Updating it for all states

		if (np.allclose(value_function, value_function_prev, atol=tol, rtol=0)):
			break
		value_function = value_function_prev.copy()
...
def policy_improvement(P, nS, nA, value_from_policy, gamma=0.9):
	...
	action_list = np.zeros(16, dtype = int)
	for state in range(0, nS):
		temp_list = []
		for action in range(0, nA):
			val = 0
			for x in P[state][action]:
				val += (x[0] * (x[2] + (gamma * value_from_policy[x[1]])))
			temp_list.append(val)
		action_list[state] = np.argmax(temp_list)

	for state in range(0, nS):
		new_policy[state][action_list[state]] = 1
	...
	while True:
		for state in range(0, nS):
			...
			for action in range(0, nA):
				val = 0
				for x in P[state][action]:
					val += (x[0] * (x[2] + (gamma * V[x[1]])))

				temp_list.append(val)
			V_new[state] = np.max(temp_list)
			action_list[state] = np.argmax(temp_list)

		if (np.allclose(V_new, V, atol=tol, rtol=0)):
			for state in range(0, nS):
				policy_new[state][action_list[state]] = 1
			break
...
def render_single(env, policy, render = False, n_episodes=100):
	...
	for _ in range(n_episodes):
		...
		while not done:
			...
			ob,reward,done,info= env.step(np.argmax(policy[ob]))
